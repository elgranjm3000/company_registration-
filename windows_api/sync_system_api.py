        ...
        try:
            ...
            if "authentication failed" in error_msg or "password authentication failed" in error_msg:
                messagebox.showerror(
                    ...
                    "Verifica:\n"
                    "• El usuario esté correcto\n"
                    "• La contraseña esté correcta\n"
                    "• El usuario tenga permisos para esta base de datos"
                )
            # Rol/usuario no existe
            elif "role" in error_msg and "does not exist" in error_msg:
                messagebox.showerror(
                    "❌ Usuario Incorrecto",
                    f"El usuario '{user}' no existe en PostgreSQL.\n\n"
                    f"Verifica que el nombre de usuario sea correcto."
                )
            # Base de datos no existe
            elif "database" in error_msg and "does not exist" in error_msg:
                messagebox.showerror(
                    "❌ Base de Datos No Existe",
                    f"La base de datos '{database}' no existe.\n\n"
                    f"Verifica:\n"
                    f"• El nombre de la base de datos esté correcto\n"
                    f"• La base de datos haya sido creada en PostgreSQL"
                )
            # Conexión rechazada
            elif "connection refused" in error_msg:
                messagebox.showerror(
                    "❌ Conexión Rechazada",
                    f"No se pudo conectar al servidor PostgreSQL.\n\n"
                    f"Verifica:\n"
                    f"• El servidor PostgreSQL esté ejecutándose\n"
                    f"• El host '{host}' sea correcto\n"
                    f"• El puerto '{port}' sea correcto (por defecto: 5432)"
                )
            # Host no encontrado
            elif "could not translate host name" in error_msg or "name or service not known" in error_msg:
                messagebox.showerror(
                    "❌ Host No Encontrado",
                    f"No se pudo encontrar el host '{host}'.\n\n"
                    f"Verifica:\n"
                    f"• El nombre del host sea correcto\n"
                    f"• El servidor esté accesible desde la red\n"
                    f"• Tengas conexión a internet (si es un servidor remoto)"
                )
            # Puerto inválido
            elif "invalid port" in error_msg or "port" in error_msg and "out of range" in error_msg:
                messagebox.showerror(
                    "❌ Puerto Inválido",
                    f"El puerto '{port}' no es válido.\n\n"
                    f"El puerto debe ser un número entre 1 y 65535.\n"
            ...
            # Timeout
                ...
                messagebox.showerror(
                    ...
                    f"La conexión a PostgreSQL tardó demasiado.\n\n"
                    f"Verifica:\n"
                    f"• El servidor esté accesible desde la red\n"
                    f"• No haya un firewall bloqueando la conexión\n"
                    f"• El servidor PostgreSQL esté ejecutándose"
                )
            # Demasiadas conexiones
                ...
                messagebox.showerror(
                    ...
                    f"El servidor PostgreSQL ha alcanzado el límite de conexiones.\n\n"
                    f"Contacta al administrador del servidor para aumentar el límite de conexiones."
                )
            # Otros errores de conexión
                ...
                messagebox.showerror(
                    ...
                    f"• Database: {database}\n"
                    f"• Usuario: {user}\n\n"
                    f"Error técnico: {str(e)[:150]}"
                )

        except ValueError as e:
            # Error de conversión de puerto a número
            ...
            messagebox.showerror(
                "❌ Error Inesperado",
                f"Ocurrió un error inesperado al conectar a PostgreSQL.\n\n"
                f"Error: {str(e)[:200]}"
            )
            self.log(f"❌ PostgreSQL: {str(e)}")
...
def authenticate_user_tray(config):
    ...
    # Botones - usar tk.Button para mejor visibilidad
    login_btn = tk.Button(
        button_frame,
        text="✅ Iniciar Sesión",
        command=do_login,
