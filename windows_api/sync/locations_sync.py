"""
Locations Sync Module
Sincronizador de locations de PostgreSQL a API REST
"""
...
class LocationsSync(BaseSync):
    """
    Sincronizador de locations usando API REST.

    Flujo:
    1. Detecta cambios en tabla 'location' de PostgreSQL
        ...
        try:
            ...
            if count_in_hashes == 0:
                ...
                if count_pending == 0:
                    ...
                    if eliminados:
                        self.info(f"{len(eliminados)} locations eliminados detectados")
                    else:
                        self.info("No hay locations con pending_sync")
                    return cambios

                self.info(f"Se encontraron {count_pending} locations con pending_sync")
            ...
            locations = self.pg_cursor.fetchall()

            self.info(f"Se recuperaron {len(locations)} locations de PostgreSQL")
            ...
            self.info(
                f"Cambios en locations: {len(cambios['nuevos'])} nuevos, "
                f"{len(cambios['modificados'])} modificados, "
                f"{len(cambios['eliminados'])} eliminados"
            )

        except Exception as e:
            self.error(f"Error detectando cambios en locations: {e}")
            import traceback
    ...
    def sync_to_api(self, changes: Dict[str, List]) -> bool:
        ...
        if not todos:
            self.info("No hay locations para sincronizar")
            return True

        self.info(f"Sincronizando {len(todos)} locations a la API...")

        locations_api = [self.transform_to_api(l) for l in todos]
        ...
        for attempt in range(max_retries):
            try:
                ...
                if self.stats['errors'] == 0:
                    self.info(f"Locations sincronizados: {self.stats['created']} creados, "
                              f"{self.stats['updated']} actualizados")
                    return True
                else:
                    self.error(f"Errores sincronizando locations: {self.stats['errors']}")
                    return False

            except Exception as e:
                if attempt >= max_retries - 1:
                    self.error(f"Error sincronizando locations: {e}")
                    return False
    ...
    def delete_from_api(self, deleted_items: List) -> None:
        ...
        self.info(f"Eliminando {len(deleted_items)} locations de la API...")
        ...
        try:
            ...
            self.info(f"Eliminados {self.stats['deleted']} locations de la API")
            ...
            self.error(f"Error eliminando locations: {e}")
